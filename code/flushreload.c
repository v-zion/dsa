...
void startSpying(args_t *args)
{
    ...
    for (i = 0; i < args->probe_count; i++) {
        ...
        // printf("CONTENTS AT %p: ", (void*)probe->virtual_address);
        // for (int j = 0; j < 4; j++){
        //     printf("%x ", *((unsigned int*)((int*)(probe->mapped_pointer) + j)));
        // }
        // printf("\n");

        // printf("Contents of binary: \n");
        // for (int j = 0; j < 100; j++){
        //     printf("%x ", *((int*)((int*)binary + j)));
        // }
        // printf("\n");
...
void printSlotBuffer(slot_t *buffer, unsigned long size, args_t *args)
{
    ...
    for (i = 0; i < size; i++) {
        hit = 0;
        for (j = 0; j < args->probe_count; j++) {
            if (buffer[i].probe_time[j] <= args->threshold && !any_hit) {
                printf("START\n");
                hit = 1;
                any_hit = 1;
            }
            printf("%c,%llu,%lu\n", args->probes[j].name, buffer[i].start, buffer[i].probe_time[j]);
        }
        // if (hit) {
        //     printf("|");
        //     if (buffer[i].missed > 0) {
